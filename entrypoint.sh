...
mkdir -p /tmp/backup/
rm -rf -- /tmp/backup/* 
mysqldump -u $MYSQL_USER -p"$MYSQL_PASSWORD" --all-databases --single-transaction -h $MYSQL_HOST -P $MYSQL_PORT --result-file=/tmp/backup/dump.sql --verbose $FORCE && gzip /tmp/backup/dump.sql
